...
import {loginSuccess, getTransaction, getTodayTJSent, getTodayTJReceive} from '../../actions/login';
...
class Router extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(!this.props.user.userId && nextProps.user.userId && Meteor.userId() && nextProps.user.role === 'user') {
      Meteor.subscribe('profile');
      Meteor.subscribe('Multiplier');
      Meteor.subscribe('balance');
      Meteor.subscribe('RecentTransferUser');
  ...
  render() {
    const {mUser,user:{role}} = this.props;
    const self = this;
        ...
        <AppLayout>
          <Switch>
            ...
            <Route exact path="/register/success" render={(props)=>{
                if(!self.props.register.inProgress && self.props.register.successMessage) {
                  return <Alert title="Successfully Registered" message={self.props.register.successMessage} type="success"/>
                } else {
                  props.history.push('/');
                }
              }} />
                ...
                Accounts.verifyEmail(token, function(err){
                  if (err) {
                     ...
                     props.history.push('/');
                      ...
                      NotificationManager.success("Email verified Successfully","",5000);
                      self.props.loginSuccess()
                      props.history.push('/');
                   }
                });
                return <Alert title="Email Verification" message="Please wait..." type="info"/>
            }} />
      ...
      return (
        <AdminLayout>
          <Switch>
            ...
            <Route exact path="/track-gold/:userId" component={UserBlockChain} />
      ...
      return (
        <AdminLayout>
          <Switch>
            ...
            <Route exact path="/change-password" component={ChangePassword} />
            <Route exact path="/reverse-transaction" component={ReverseTransaction} />
            <Route exact path="/login" render={(props)=>{
                props.history.push('/')
                return null;
              }} />
      ...
      return (
        <AppLayout>
          <Switch>
            ...
            <Route exact path="/transaction/:tId" component={SingleTransaction} />
